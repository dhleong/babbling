...
export default async function mediaControlCommand(opts: IMediaControlCommand) {
    ...
    try {
        ...
        switch (opts.cmd) {
            ...
            case "next":
                controls.nextQueueItem();
                break;
            case "prev":
                controls.prevQueueItem();
                break;

            case "ff":
                controls.seekRelative(requireArg(opts));
                break;
            case "rew":
                controls.seekRelative(requireArg(opts));
                break;
            case "seek":
                controls.seekTo(requireArg(opts));
                break;
            ...
            case "volume":
                controls.setVolume(requireArg(opts) / 10);
                break;
